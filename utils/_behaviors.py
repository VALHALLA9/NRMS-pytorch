...
def add_prediction_scores(
    ...
    Args:
        ...
        prediction_scores_col (str): The name of the column to store the prediction scores.
        inview_col (str): The column name containing the list of articles for which scores are assigned.

    Returns:
        pl.DataFrame: The DataFrame with the prediction scores added.
    """
    GROUPBY_ID = "_groupby_id"
    
    # Add a unique row index for grouping
    df = df.with_row_count(name=GROUPBY_ID)

    # Explode the inview column and align with scores
    exploded_df = (
        df.lazy()
        .explode(inview_col)  # Explode the list column
        .with_columns(
            pl.Series(prediction_scores_col, scores).alias(prediction_scores_col)  # Add scores
        )
    )

    # Group by _groupby_id and aggregate the columns back into lists
    aggregated_df = (
        exploded_df.group_by(GROUPBY_ID)
        .agg(
            pl.col(inview_col).alias(inview_col),  # Group the exploded column back into lists
            pl.col(prediction_scores_col).alias(prediction_scores_col),  # Group scores back into lists
        )
    )

    # Join the aggregated data back to the original DataFrame
    result_df = df.lazy().join(aggregated_df, on=GROUPBY_ID, how="left")

    # Drop the temporary _groupby_id column
    return result_df.collect().drop(GROUPBY_ID)
